...
package com.lamp.decoration.foundation.network.http.netty;
...
import com.lamp.decoration.foundation.network.http.handler.DefaultCall;
import com.lamp.decoration.foundation.network.http.handler.Http11Factory;
import com.lamp.decoration.foundation.network.http.handler.Http11Factory.ChannelWrapper;
import com.lamp.decoration.foundation.network.http.model.ModelManage;
import com.lamp.foundation.api.call.Callback;
import com.lamp.foundation.api.constant.LightConstant;
import com.lamp.foundation.api.http.interceptor.Interceptor;
import com.lamp.foundation.api.http.response.Response;
...
public class NettyClient {
    ...
    public void error(AsyncReturn asyncReturn, Throwable throwable) {
        ...
        for (Interceptor interceptor : asyncReturn.interceptorList()) {
            interceptor.handlerError(throwable, asyncReturn.handleMethod().getRequestInfo().requestWrapper(), asyncReturn.getArgs());
        }
    }
        ...
        private void returnHandle(ChannelHandlerContext ctx) {
            ...
            if (Objects.nonNull(throwable)) {
                ...
                if (connect.writerIndex() > 0) {
                    object = asyncReturn.serialize().deserialization(connect.array(), asyncReturn.handleMethod().getRequestInfo().getReturnClazz());
                    this.release();
                }
            }
