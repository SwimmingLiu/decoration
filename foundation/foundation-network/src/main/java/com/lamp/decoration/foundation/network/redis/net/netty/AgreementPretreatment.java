...
public class AgreementPretreatment {
    ...
    public final static <T> void ListReferenceAgreementPretreatment(OutputStream out, List<DataConversion> list, List<T> objectList, int num,
        KeyCreate<T> keyCreate) throws IOException {
        ...
        for (; ; ) {
            if (keyCreate != null) {
                ...
                keyCreate.getKeySuffixBuffer(objectList.get(j), bb);
                ProtocolUtil.write(out, bb);
            } else {
                ProtocolUtil.write(out, JsonDeToSerialize.SERIALIZE_DEFAULT.execute(objectList.get(j)));
            }
            if (j++ == i) {
                break;
            }
    ...
    public final static <T> void HashReferenceAgreementPretreatment(OutputStream out, List<DataConversion> list, List<?> objectList,
        KeyCreate<T> keyCreate) throws IOException {
        ProtocolUtil.write(out, list.get(0).getWriteByteBuffer());
        int i = objectList.size(), len = 0;
        T o;
        ByteBuffer bb;
        KeyCreate value = keyCreate.getKeyCreateValue();
        for (; ; ) {
            o = (T) objectList.get(len++);
            bb = DataConversion.getBuffer();
            if (value != null) {
                value.getKeySuffixBuffer(o, bb);
                ProtocolUtil.write(out, bb);
            }
