...
public class ListCommandsTest extends ConnectionFactoryTest {
	...
	public void getData(){
		...
		BlockListResult< TestEntity > block;
		
		block = lc.blpop( testEntity.getId( ) );
		System.out.println(  "blpop long return : "  + block) ;
		
		block = lc.blpop( testEntity.getId( ) + "");
		System.out.println(  "blpop String return : "  + block) ;
		
		block = lc.blpop( testEntity );
		System.out.println(  "blpop String return : "  + block) ;
		...
		//TODO 开始出错 
		block = lc.brpop( testEntity.getId( ) );
		System.out.println(  "brpop long return : "    + block) ;
		
		block = lc.brpop( testEntity.getId( ) + "");
		System.out.println(  "brpop String return : "  + block) ;
		
		block = lc.brpop( testEntity );
		System.out.println(  "brpop String return : "  + block) ;
	}
